...
class GA:
    def __init__(self,individualSize,populationSize):
        ...
        self.indivdualSize = individualSize
        ...
        for individual in self.population:
            ...
            if individualLength>0:
                ...
                while i<individualLength:
                    roluetteWheel.append(individual)
                    i=i+1
        random.shuffle(roluetteWheel)
        parentIndices = []
        i=0

        while i< self.populationSize:
            parentIndices.append(roluetteWheel[random.randint(0,len(roluetteWheel)-1)])
            i=i+1
        newGeneration = dict()
        i=0
        while i< self.populationSize:
    ...
    def generateChildren(self,crossOverProbability):
        ...
        while i<numberOfPairs:
            crossOverPoint = random.randint(0,self.indivdualSize-1)
    ...
    def mutateChildren(self,mutationProbability):
        numberOfBits = round(mutationProbability*self.populationSize*self.indivdualSize)
        totalIndices = list(range(0,self.populationSize*self.indivdualSize))
        random.shuffle(totalIndices)
        swapLocations = random.sample(totalIndices,numberOfBits)

        for loc in swapLocations:
            individualIndex = math.floor(loc/self.indivdualSize)
            bitIndex = math.floor(loc%self.indivdualSize)
