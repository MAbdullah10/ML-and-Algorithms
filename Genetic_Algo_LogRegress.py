...
class GA:
    ...
    def updatePopulationFitness(self): # Calculates and updates the fitness of each individual in the population.
        self.totalFitness = 0
        # Loops through each individual, computes its fitness using the logistic_regression_fitness function, updates the individual's fitness, and adds it to the total fitness.
        for individual in self.population:
            fitness = logistic_regression_fitness(X_train_scaled, y_train, self.population[individual][0])
...
def logistic_regression_fitness(X_train, y_train, coefficients): # Computes the fitness of an individual based on the log loss (cross-entropy loss) of the predictions
    z = np.dot(X_train, coefficients[1:]) + coefficients[0] # X_train is multiplied by the coefficients (excluding the bias term), and the bias term is added to obtain the logit.
    y_pred = sigmoid(z) # Apply sigmoid to get probabilities.
    return log_loss(y_train, y_pred) # Return the log loss as fitness.

data = pd.read_csv("Linear Regression - Sheet1.csv")
data.dropna(inplace=True) # Drops any rows with missing values.
# Assuming 'Y' is binary for logistic regression (e.g., 0 or 1)
X = data[['X']].values
y = data['Y'].values
...
for i in range(100): 
    ...
    print("Best fitness (Log Loss):", best_fitness)
...
plt.ylabel('Best Fitness (Log Loss)')
plt.grid(True)
plt.show()
